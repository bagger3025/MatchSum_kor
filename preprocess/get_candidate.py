...
_ROUGE_PATH = '/home/vaiv2021/pyrouge/tools/ROUGE-1.5.5'
...
@curry
def get_candidates(tokenizer, cls, sep_id, idx, dacon=False):
    ...
    if dacon:
        ...
        data['text'] = original_data[idx]['article_original']
        data['summary'] = original_data[idx]['article_original'][:3]
    else:
...
def get_candidates_mp(args):
    ...
    with mp.Pool() as pool:
        list(pool.imap_unordered(get_candidates(tokenizer, cls, sep_id, dacon=dacon), range(n_files), chunksize=64))
    ...
    os.system(f"rm {args.write_path}")
...
if __name__ == '__main__':
    ...
    parser.add_argument('--dacon', type=str, default='f',
        help="if dacon is True, --dacon y")

    args = parser.parse_args()
    assert args.tokenizer in ['bert', 'kobert', 'roberta']
