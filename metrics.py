...
class MatchRougeMetric(MetricBase):
    def __init__(self, data, dec_path, ref_path, n_total, to_csv=None, score=None):
    ...
    def get_metric(self, reset=True):
        ...
        if self.to_csv is not None:
            file = open(self.to_csv, mode="w", encoding="UTF-8", newline="")
        ...
        for i, ext in enumerate(self.ext):
            ...
            if self.to_csv is None:
                ...
                summary = []
                for j in sent_ids:
                    summary.append(self.data[i]['text'][j])
                writer.writerow([self.data[i]['id'], '\n'.join(summary)])
